...
namespace services::nvs {
    namespace {
        ...
        constexpr uint8_t DEFAULT_THEME         = static_cast<uint8_t>(Theme::DEFAULTS);
        constexpr uint8_t DEFAULT_UNITS         = static_cast<uint8_t>(Units::METRIC);
    }
    ...
    Theme getTheme() {
        ...
        const uint8_t value = _prefs.getUChar(KEY_THEME, DEFAULT_THEME);
    ...
    bool setTheme(Theme theme) {
        ...
        switch (theme) {
            ...
            case Theme::NIGHT:
            case Theme::HIGHS:
                return _prefs.putUChar(KEY_THEME, static_cast<uint8_t>(theme)) > 0;
        }
        return false;
    }
    ...
    Units getUnits() {
        if (!_READY && !begin())
            { return Units::METRIC; }
        const uint8_t value = _prefs.getUChar(KEY_UNITS, DEFAULT_UNITS);
    ...
    bool setUnits(Units units) {
        ...
        switch (units) {
            case Units::METRIC:
            case Units::IMPERIAL:
                return _prefs.putUChar(KEY_UNITS, static_cast<uint8_t>(units)) > 0;
        }
        return false;
    }
